...
def save_results(file_path, avg_lat, avg_lon, avg_height, avg_X, avg_Y, avg_Z):
    ...
    try:
        ...
        with open(output_file_path, 'w', encoding='gbk') as file:
            ...
            file.write(f"---------------------------------------------\n")
            file.write(f"Lat(dms)：{lat_dms[0]}°{lat_dms[1]}′{lat_dms[2]:.9f}″\n")
            file.write(f"Lon(dms)：{lon_dms[0]}°{lon_dms[1]}′{lon_dms[2]:.9f}″\n")
            file.write(f"Height(m)：{avg_height:.4f}\n")
            file.write(f"---------------------------------------------\n")
            file.write(f"Lat(deg)：{avg_lat:.9f}\n")
            file.write(f"Lon(deg)：{avg_lon:.9f}\n")
            file.write(f"Height(m)：{avg_height:.4f}\n")
            file.write(f"---------------------------------------------\n")
            file.write(f"X(ECEF/m)：{avg_X:.9f}\n")
            file.write(f"Y(ECEF/m)：{avg_Y:.9f}\n")
            file.write(f"Z(ECEF/m)：{avg_Z:.9f}\n")
    ...
    if file_paths:
        for file_path in file_paths:
            ...
            if measurements:
                ...
                print(f"平均纬度(dms): {lat_dms[0]}°{lat_dms[1]}′{lat_dms[2]:.9f}″")
                print(f"平均经度(dms): {lon_dms[0]}°{lon_dms[1]}′{lon_dms[2]:.9f}″")
                print(f"平均纬度(deg): {avg_lat:.9f}°")
                print(f"平均经度(deg): {avg_lon:.9f}°")
                print(f"平均大地高: {avg_height:.4f}m")
                print(f"平均X坐标: {avg_X:.9f}m")
                print(f"平均Y坐标: {avg_Y:.9f}m")
                print(f"平均Z坐标: {avg_Z:.9f}m")
